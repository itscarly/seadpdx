...
async function init() {
  ...
  const cheapestExcluded = report.seattle.excluded[0];
  ...
  document.getElementById("trackerSubhead").textContent = `${benchmarkPrice} | ${report.seattle.eligible.length} live-verified alternatives under cap`;
  ...
  document.getElementById("benchmarkPanel").innerHTML = `
    <article class="summary-card">
      ...
      <p><strong>Total:</strong> ${formatMoney(benchmark.trueTotalCost)}</p>
  ...
  document.getElementById("watchSummary").innerHTML = `
    <article class="summary-card">
      ...
      <p>${cheapestExcluded ? `Cheapest live direct alternative: ${escapeHtml(cheapestExcluded.name)} at ${formatMoney(cheapestExcluded.trueTotalCost)}.` : ""}</p>
    </article>
  `;
